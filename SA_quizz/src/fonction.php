<?php
		...
		//function pour définir les régles "c" "e" et "f"
			...
			//function pour définir si nous avons un entier superieur à 5
			function SupCinq($chaine){
				if (preg_match("#^(([5-9][0-9]*)|([1-4][0-9]+))$#", $chaine))
					return true;
				return false;
			}
		//Foncrion pour verifier si les password sont les mêmes
			function Correspondance($number,$confirm){
				if ($number==$confirm)
					return true;
			}
		...
		//Fonction pour définir la règle i et j
			function RegleCommentaire($commentaire){
